...
# 2 "<built-in>" 2
# 1 "charger_discharger.c" 2
# 14 "charger_discharger.c"
...
void pid(float feedback, float setpoint)
{
 ...
 pidi += (ki * er);
    ...
    if (pidt >= 300.0) pidt = 300.0;
    else if (pidt <= 50.0) pidt = 50.0;
}
...
void calculate_avg()
{
    switch(count)
    {
        case 1024:
            ...
            break;
        case 0:
            iavg = ((iacum >> 10) + ((iacum >> 9) & 0x01));
            vavg = ((vacum >> 10) + ((vacum >> 9) & 0x01));
            break;
        default:
            iacum += (uint24_t) i;
            vacum += (uint24_t) v;
